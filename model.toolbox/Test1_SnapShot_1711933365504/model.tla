    ...
    process (planing_base_task \in 1..Len(Input_priority))
    {
         plan_loop_B:
             while (TRUE) {
                ...
                or{
                    await /\ tasks[self].type = "A";
                        advenc[Len(advenc)+1] := Append(advenc[Len(advenc)+1], tasks[self]);
...
*)
\* BEGIN TRANSLATION (chksum(pcal) = "5084a0fc" /\ chksum(tla) = "ccf08a40")
VARIABLES ready, runProcess, run, prioritys, advenc, tasks

vars == << ready, runProcess, run, prioritys, advenc, tasks >>
...
planing_base_task(self) == \/ /\ /\ tasks[self].status = "suspended"
                              ...
                              /\ ready' = ready + 1
                              /\ UNCHANGED <<runProcess, run, advenc>>
                           \/ /\ /\ tasks[self].status = "ready"
                                 /\ run < 1
                                 /\ tasks[self].type = "B"
                                 /\ Head(prioritys[tasks[self].prior]) = self
                           ...
                           \/ /\ /\ tasks[self].status = "running"
                              ...
                              /\ run' = run-1
                              /\ runProcess' = 0
                              /\ UNCHANGED <<ready, prioritys, advenc>>
                           \/ /\ /\ tasks[self].status = "ready"
                                 /\ Head(prioritys[tasks[self].prior]) = self
                                 /\ \A j \in (tasks[self].prior + 1)..3 : Len(prioritys[j]) = 0
                                 /\ tasks[self].type = "B"
                              ...
                              /\ tasks' = [tasks EXCEPT ![runProcess].status = "ready",
                                                        ![self].status = "running"]
                              /\ runProcess' = self
                              /\ UNCHANGED <<ready, run, advenc>>
                           \/ /\ /\ tasks[self].type = "A"
                              /\ advenc' = [advenc EXCEPT ![Len(advenc)+1] = Append(advenc[Len(advenc)+1], tasks[self])]
                              /\ UNCHANGED <<ready, runProcess, run, prioritys, tasks>>
...
\* Last modified Mon Apr 01 04:02:36 MSK 2024 by adeli
