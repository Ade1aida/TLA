...
(*--algorithm tlaLab_algorithm {
    ...
    process (baseTask \in 1..Len(Proc2))
    {
        baseLoop:
            while (TRUE) {
    ...
    process (advancedTask \in 1..Len(Proc1))
    {
        advancedLoop:
            while (TRUE) {
...
*)
\* BEGIN TRANSLATION (chksum(pcal) = "73d1dfd8" /\ chksum(tla) = "ec031411")
VARIABLES ready, run, states, prioritys

vars == << ready, run, states, prioritys >>

ProcSet == (1..Len(Proc2)) \cup (1..Len(Proc1))
...
Next == (\E self \in 1..Len(Proc2): baseTask(self))
           \/ (\E self \in 1..Len(Proc1): advancedTask(self))
...
\* Last modified Fri Mar 29 15:13:02 MSK 2024 by adeli
